...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Allocative.swiftmodule/`
...
//      `import Allocative` above would fail at compile time.
//
//   2. The static archive `libAllocative.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Allocative" }` and
//      `flattenPackage = "io.github.kotlinmania.allocative"` configuration in
...
//      via the `AllocativeLibrary` product.
//
...
final class AllocativeExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Allocative swift module imported cleanly")
    }
}
