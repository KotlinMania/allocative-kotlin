# Claude Code Project Instructions — allocative-kotlin

## Project Overview

This is **allocative-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`allocative`](https://crates.io/crates/allocative).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/allocative/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/allocative>
